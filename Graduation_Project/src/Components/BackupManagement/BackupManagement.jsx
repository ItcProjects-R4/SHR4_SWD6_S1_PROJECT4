        ...
        <div className={Style.panel}>
            ...
            <div className={Style.cards}>
                <div className={Style.card}>
                    ...
                    <button className={Style.primaryBtn} onClick={() => setModalType("backup-products")}>Backup</button>

                    <button className={Style.secondaryBtn} onClick={() => setModalType("restore-products")}>Restore</button>
                </div>

                <div className={Style.card}>
                    <h4>Categories</h4>
                    <button className={Style.primaryBtn} onClick={() => setModalType("backup-categories")}>Backup</button>
                    <button className={Style.secondaryBtn} onClick={() => setModalType("restore-categories")}>Restore</button>
                </div>
            </div>
        </div>

        {modalType && (
            <div className={Style.modalOverlay} onClick={() => !loading && setModalType(null)}>
                <div className={Style.modal} onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className={Style.modalBody}>
                        {loading ? (
                            ...
                            <p className={Style.successText}>Done successfully</p>
                        ) : (
                            <p>
                    ...
                    <div className={Style.modalFooter}>
                        {!loading && !done && (
                            <>
                                <button className={Style.cancelBtn} onClick={() => setModalType(null)}>Cancel</button>
                                <button className={Style.confirmBtn} onClick={handleAction}>Confirm</button>
                            </>
                        )}

                        {done && (
                            <button className={Style.confirmBtn} onClick={() => { setModalType(null); setDone(false);}}>Close</button>
                        )}
                    </div>
                </div>
            </div>
