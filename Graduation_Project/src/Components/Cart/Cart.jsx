...
export default function Cart() {
  ...
  let imgStyle = { width: "100%", maxWidth: window.innerWidth <= 768 ? "100%" : "100px", height: window.innerWidth <= 768 ? "180px" : "100px", objectFit: "cover", borderRadius: "12px", background: isLight ? "#f3f4f6" : "#1e2a3a" };
            ...
            <div key={item.id} style={cardStyle}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                  ...
                  <button className="bg-transparent" style={{ border: "none", color: "#e74c3c", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", fontSize: "14px" }} onClick={() => setConfirmId(item.id)}>
                    <i className="fa-solid fa-trash"></i> Remove
                  </button>
                </div>
              </div>
              <span style={{ ...itemTotalStyle, alignSelf: window.innerWidth <= 768 ? "flex-end" : "unset", marginTop: window.innerWidth <= 768 ? "8px" : "0", }}>${(item.price * item.quantity).toFixed(2)}</span>
            </div>
          ))}
        </div>
      ...
      <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
        <div style={{ background: isLight ? "#ffffff" : "#1a2235", borderRadius: "12px", padding: "32px", maxWidth: "600px", width: "90%" }}>
          ...
          <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
            ...
            <button onClick={() => { dispatch(removeFromCart({ id: confirmId, userId: cartUserId })); setConfirmId(null); }} style={{ background: "#e74c3c", border: "none", color: isLight ? "#111827" : "#ffffff", padding: "10px 20px", borderRadius: "8px", cursor: "pointer" }}>Remove</button>
          </div>
        </div>
      </div>
    )}
